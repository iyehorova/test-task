...
export const ProductItem: React.FC<Props> = ({ product }) => {
  ...
  return (
    <div className="row align-items-center text-primary">
      <span className="col-7 col-sm-3 col-md-3 d-flex flex-column">
        <span className="text-decoration-underline">{product.title}</span>
        <span className="fs-8 text-light">{product.serialNumber}</span>
      </span>
      <span className="col-5 col-sm-2 col-md-2 fs-7">
        {product.type.toLowerCase()}
      </span>

      <span className="col-12 d-sm-none border border-muted"></span>

      <span className="col-4 col-sm-2 col-md-3 col-lg-2 d-flex flex-column align-items-center">
        <span className="d-flex justify-content-between w-100">
          <span className="fs-8 text-light">from</span>{' '}
          <span className="d-none d-md-block fs-7">{guaranteeStart}</span>
          <span className="d-md-none fs-7">{guaranteeStartShort}</span>
        </span>
        <span className="d-flex justify-content-between w-100">
          <span className="fs-8 text-light">to</span>{' '}
          <span className="d-none d-md-block fs-7">{guaranteeEnd}</span>
          <span className="d-md-none fs-7">{guaranteeEndShort}</span>
        </span>
      </span>
      ...
      <span className="col-3 col-sm-2">{product.orderInfo.title}</span>
